...
var Paul_Pio = function (prop) {
    ...
    var action = {
        ...
        welcome: function () {
            if (document.referrer !== "" && document.referrer.indexOf(current.root) === -1) {
                ...
                prop.content.referer ? modules.message(prop.content.referer.replace(/%t/, "“" + referrer.hostname + "”")) : modules.message("欢迎来自 “" + referrer.hostname + "” 的朋友！");
            }
            else if (prop.tips) {
        ...
        buttons: function () {
            ...
            elements.skin.onclick = function () {
                that.model = loadlive2d("pio", prop.model[modules.idol()], model => {
                    ...
                    prop.content.skin && prop.content.skin[1] ? modules.message(prop.content.skin[1]) : modules.message("新衣服真漂亮~");
                });
            };
            elements.skin.onmouseover = function () {
                prop.content.skin && prop.content.skin[0] ? modules.message(prop.content.skin[0]) : modules.message("想看看我的新衣服吗？");
            };
            ...
            elements.night.onclick = function () {
                if(handsome_UI.mode === "light"){
                    eval(handsome_UI.dark_mode())
                }else{
                    eval(handsome_UI.light_mode())
                }
        ...
        custom: function () {
            prop.content.custom.forEach((item) => {
                ...
                for (let i = 0; i < el.length; i++) {
                    ...
                    }else if(item.type === "category-parent"){
                        el[i].onmouseover = (ev) => {
                            const text = ev.currentTarget.title || ev.currentTarget.innerText;
                            if (!ev.currentTarget.className.includes("active")) {
                                modules.message("想了解一下 %t 这个分类吗？".replace(/%t/, "“" + text.replace(/.*\n/, "") + "”"));
                            }
                        }
                    }else if(item.type === "category-child"){
                        el[i].onmouseover = (ev) => {
                            const text = ev.currentTarget.title || ev.currentTarget.innerText;
                            modules.message("想了解一下 %t 这个子分类吗？".replace(/%t/, "“" + text.replace(/.*\n/, "") + "”"));
                        }
                    }
                    else if (item.text) {
                        el[i].onmouseover = () => {
    ...
    this.initHidden = function () {
        current.body.classList.toggle("hidden");
        dialog.classList.remove("active");
